...
export class MapComponent implements OnInit {
  ...
  L.marker([-3.25, 33.30], {icon: myIcon}).addTo(this.map)
    ...
    L.marker([-1.25, 34.30], {icon: myIcon}).addTo(this.map)
    .bindPopup('If I have found these (tips) helpful, for sure they should be for all parents and caregivers in our village. They are also applicable to people of all faiths, and I will highly recommend to the village head so we could advocate across our community. They are timely.<br> - Counsellor to village Headman Mphamba in Malawi')
    .openPopup();
